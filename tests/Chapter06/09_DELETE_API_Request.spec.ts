...
import updateAPIRequest from '../../test-data/api_requests/UPDATE_API_Request.json';
...
test('Create DELETE API Request in Playwright', { tag: ['@PlaywrightWithJenkins'] }, async ({ request }) => {
    ...
    //Create a PATCH API request to update the booking details using the booking id and token generated from the previous steps
    const updateAPIResponse = await request.patch(`/booking/${bookingId}`, { 
    ...
    //validate the PATCH API response status code and status text
    expect(updateAPIResponse.status()).toBe(200);
    expect(updateAPIResponse.statusText()).toBe("OK");
    expect(updateAPIResponse.headers()['content-type']).toContain('application/json');

    //Convert API response to JSON
    const updateAPIJSONResponse = await updateAPIResponse.json();

    //Print the UPDATE API response body in the console as String with better formatting for better readability in the console      
    console.log('UPDATE API Response after update: '+JSON.stringify(updateAPIJSONResponse,null,2));
   
    //--------------------------------------------------------------------------------------------------

    //Create Delete API request to delete the booking using the booking id and token generated from the previous steps
    ...
    expect(deleteAPIResponse.statusText()).toBe("Created");
     ...
     //Print the DELETE API response body in the console as String with better formatting for better readability in the console      
    console.log('DELETE API Response body after delete: '+ await deleteAPIResponse.body());
});