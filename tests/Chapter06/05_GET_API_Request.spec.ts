...
//Write a test
test('Create GET API Request using Playwright', async ({ request }) => {

    //Create a GET API request body dynamically by replacing the placeholders in the template with actual values
    //Reading JSON file
    ...
    //GET API request to fetch the booking details using the booking id from the response of POST API request
    ...
    console.log('Response Body from GET API: '+JSON.stringify(getAPIJSONResponse,null,2));
});